...
volatile uint8 USER_PORT_curStatus; /* Status byte */
uint8 USER_PORT_fsmState;           /* FSM state   */

/* Variables intended to be used with Buffer 1: Primary slave address */
volatile uint8 * USER_PORT_dataBuffer1; /* Pointer to data buffer 1 */
uint16 USER_PORT_bufSizeBuf1; /* Size of buffer 1 in bytes      */
uint16 USER_PORT_protectBuf1; /* Start index of write protected area buffer 1 */
uint16 USER_PORT_offsetBuf1; /* Current offset within buffer 1 */
uint16 USER_PORT_indexBuf1;  /* Current index within buffer 1  */

#if(USER_PORT_SECONDARY_ADDRESS_ENABLE_CONST)
    uint8 USER_PORT_addrBuf1; /* Primary slave address. Used for software comparison   */
    uint8 USER_PORT_addrBuf2; /* Secondary slave address. Used for software comparison */

    /* Variables intended to be used with Buffer 2: Primary slave address */
    volatile uint8 * USER_PORT_dataBuffer2; /* Pointer to data buffer 2 */
    uint16 USER_PORT_bufSizeBuf2; /* Size of buffer 2 in bytes      */
    uint16 USER_PORT_protectBuf2; /* Start index of write protected area buffer 2 */
    uint16 USER_PORT_offsetBuf2; /* Current offset within buffer 2 */
    uint16 USER_PORT_indexBuf2;  /* Current index within buffer 2  */
...
***************************************/
...
#if(USER_PORT_SECONDARY_ADDRESS_ENABLE_CONST)
    static uint32 USER_PORT_EzI2CUpdateRxMatchReg(uint32 addr1, uint32 addr2);
#endif /* (USER_PORT_SECONDARY_ADDRESS_ENABLE_CONST) */
    ...
    ***************************************/
    ...
    const USER_PORT_EZI2C_INIT_STRUCT USER_PORT_configEzI2C =
    {
        USER_PORT_EZI2C_CLOCK_STRETCHING,
        USER_PORT_EZI2C_MEDIAN_FILTER_ENABLE,
        USER_PORT_EZI2C_NUMBER_OF_ADDRESSES,
        USER_PORT_EZI2C_PRIMARY_SLAVE_ADDRESS,
        USER_PORT_EZI2C_SECONDARY_SLAVE_ADDRESS,
        USER_PORT_EZI2C_SUB_ADDRESS_SIZE,
        USER_PORT_EZI2C_WAKE_ENABLE,
        USER_PORT_EZI2C_BYTE_MODE_ENABLE
    };

    /*******************************************************************************
    * Function Name: USER_PORT_EzI2CInit
    ********************************************************************************
    *
    * Summary:
    ...
    *******************************************************************************/
    void USER_PORT_EzI2CInit(const USER_PORT_EZI2C_INIT_STRUCT *config)
    {
        ...
        {
            /* Configure pins */
            USER_PORT_SetPins(USER_PORT_SCB_MODE_EZI2C, USER_PORT_DUMMY_PARAM,
                                                                      USER_PORT_DUMMY_PARAM);

            /* Store internal configuration */
            USER_PORT_scbMode       = (uint8) USER_PORT_SCB_MODE_EZI2C;
            USER_PORT_scbEnableWake = (uint8) config->enableWake;
            USER_PORT_scbEnableIntr = (uint8) USER_PORT_SCB_IRQ_INTERNAL;

            USER_PORT_numberOfAddr  = (uint8) config->numberOfAddresses;
            USER_PORT_subAddrSize   = (uint8) config->subAddrSize;
            ...
            USER_PORT_CTRL_REG     = USER_PORT_GET_CTRL_BYTE_MODE  (config->enableByteMode)    |
                                            USER_PORT_GET_CTRL_ADDR_ACCEPT(config->numberOfAddresses) |
                                            USER_PORT_GET_CTRL_EC_AM_MODE (config->enableWake);

            USER_PORT_I2C_CTRL_REG = USER_PORT_EZI2C_CTRL;

            /* Configure RX direction */
            USER_PORT_RX_CTRL_REG = USER_PORT_EZI2C_RX_CTRL |
                                        USER_PORT_GET_RX_CTRL_MEDIAN(USER_PORT_DIGITAL_FILTER_DISABLE);
                                                ;
            USER_PORT_RX_FIFO_CTRL_REG = USER_PORT_CLEAR_REG;

            /* Set default address and mask */
            if(USER_PORT_EZI2C_PRIMARY_ADDRESS == config->numberOfAddresses)
            {
                USER_PORT_RX_MATCH_REG = USER_PORT_EzI2CUpdateRxMatchReg(config->primarySlaveAddr,
            ...
            USER_PORT_TX_CTRL_REG      = USER_PORT_EZI2C_TX_CTRL;
            USER_PORT_TX_FIFO_CTRL_REG = ((0u != (config->enableClockStretch)) ?
                                                 USER_PORT_CLEAR_REG : USER_PORT_EZI2C_TX_FIFO_CTRL);
           ...
           USER_PORT_INTR_SPI_EC_MASK_REG = USER_PORT_NO_INTR_SOURCES;
            ...
            USER_PORT_INTR_SLAVE_MASK_REG  = USER_PORT_EZI2C_INTR_SLAVE_MASK;
            ...
            (void) CyIntSetVector(USER_PORT_ISR_NUMBER, (0u != (config->enableClockStretch)) ?
                                                                      (&USER_PORT_EZI2C_STRETCH_ISR) :
                                                                      (&USER_PORT_EZI2C_NO_STRETCH_ISR));

            if(0u != (config->enableClockStretch))
            {
                /* Configure interrupt sources */
                USER_PORT_INTR_SLAVE_MASK_REG |= USER_PORT_INTR_SLAVE_I2C_ADDR_MATCH;
                USER_PORT_INTR_RX_MASK_REG     = USER_PORT_NO_INTR_SOURCES;
            }
            else
            {
                /* Enable Auto ACK/NACK features */
                USER_PORT_I2C_CTRL_REG |= USER_PORT_EZI2C_CTRL_AUTO;

                /* Configure RX interrupt source */
                USER_PORT_INTR_SLAVE_MASK_REG |= USER_PORT_INTR_SLAVE_I2C_START;
                USER_PORT_INTR_RX_MASK_REG     = USER_PORT_INTR_RX_NOT_EMPTY;
            }

            /* Configure global variables */
            USER_PORT_fsmState = USER_PORT_EZI2C_FSM_IDLE;

            USER_PORT_curStatus  = 0u;
            USER_PORT_indexBuf1  = 0u;
            USER_PORT_offsetBuf1 = 0u;
            USER_PORT_indexBuf2  = 0u;
            USER_PORT_offsetBuf2 = 0u;

            USER_PORT_addrBuf1 = (uint8) config->primarySlaveAddr;
            USER_PORT_addrBuf2 = (uint8) config->secondarySlaveAddr;
        }
    }
    ...
    *******************************************************************************/
    ...
    {
        /* Configure I2C interface */
        USER_PORT_CTRL_REG     = USER_PORT_EZI2C_DEFAULT_CTRL;
        USER_PORT_I2C_CTRL_REG = USER_PORT_EZI2C_DEFAULT_I2C_CTRL;
        ...
        /* Configure RX direction */
        USER_PORT_RX_CTRL_REG      = USER_PORT_EZI2C_DEFAULT_RX_CTRL;
        USER_PORT_RX_FIFO_CTRL_REG = USER_PORT_EZI2C_DEFAULT_RX_FIFO_CTRL;

        /* Set default address and mask */
        USER_PORT_RX_MATCH_REG     = USER_PORT_EZI2C_DEFAULT_RX_MATCH;

        /* Configure TX direction */
        USER_PORT_TX_CTRL_REG      = USER_PORT_EZI2C_DEFAULT_TX_CTRL;
        USER_PORT_TX_FIFO_CTRL_REG = USER_PORT_EZI2C_DEFAULT_TX_FIFO_CTRL;
        ...
        (void) CyIntSetVector(USER_PORT_ISR_NUMBER, &USER_PORT_EZI2C_STRETCH_ISR);
    #else
        (void) CyIntSetVector(USER_PORT_ISR_NUMBER, &USER_PORT_EZI2C_NO_STRETCH_ISR);
        ...
        /* Configure interrupt sources */
    #if (!USER_PORT_CY_SCBIP_V1)
        USER_PORT_INTR_SPI_EC_MASK_REG = USER_PORT_EZI2C_DEFAULT_INTR_SPI_EC_MASK;
    #endif /* (!USER_PORT_CY_SCBIP_V1) */

        USER_PORT_INTR_I2C_EC_MASK_REG = USER_PORT_EZI2C_DEFAULT_INTR_I2C_EC_MASK;
        USER_PORT_INTR_SLAVE_MASK_REG  = USER_PORT_EZI2C_DEFAULT_INTR_SLAVE_MASK;
        USER_PORT_INTR_MASTER_MASK_REG = USER_PORT_EZI2C_DEFAULT_INTR_MASTER_MASK;
        USER_PORT_INTR_RX_MASK_REG     = USER_PORT_EZI2C_DEFAULT_INTR_RX_MASK;
        USER_PORT_INTR_TX_MASK_REG     = USER_PORT_EZI2C_DEFAULT_INTR_TX_MASK;
        ...
        USER_PORT_addrBuf1 = USER_PORT_EZI2C_PRIMARY_SLAVE_ADDRESS;
        USER_PORT_addrBuf2 = USER_PORT_EZI2C_SECONDARY_SLAVE_ADDRESS;
    #endif /* (USER_PORT_SECONDARY_ADDRESS_ENABLE_CONST) */
    }
...
/*******************************************************************************
* Function Name: USER_PORT_EzI2CStop
********************************************************************************
*
* Summary:
...
/*******************************************************************************
* Function Name: USER_PORT_EzI2CGetActivity
********************************************************************************
*
* Summary:
...
uint32 USER_PORT_EzI2CGetActivity(void)
{
    ...
    status = USER_PORT_curStatus;

    /* Relay on address match event from HW as bus busy status */
    if(USER_PORT_EZI2C_SCL_STRETCH_DISABLE)
    {
        /* For USER_PORT_CY_SCBIP_V0 the wake is prohibited by customizer */
        if(USER_PORT_EZI2C_EC_AM_ENABLE)
        {
            status |= USER_PORT_CHECK_INTR_I2C_EC(USER_PORT_INTR_I2C_EC_WAKE_UP) ?
                        USER_PORT_EZI2C_STATUS_BUSY : 0u;
        }
        else
        {
            status |= USER_PORT_CHECK_INTR_SLAVE(USER_PORT_INTR_SLAVE_I2C_ADDR_MATCH) ?
                        USER_PORT_EZI2C_STATUS_BUSY : 0u;
        }
    ...
    USER_PORT_curStatus &= ((uint8) ~USER_PORT_EZI2C_CLEAR_STATUS);
...
/*******************************************************************************
* Function Name: USER_PORT_EzI2CSetAddress1
********************************************************************************
*
* Summary:
...
void USER_PORT_EzI2CSetAddress1(uint32 address)
{
    ...
    {
        USER_PORT_addrBuf1 = (uint8) address;

        USER_PORT_RX_MATCH_REG = USER_PORT_EzI2CUpdateRxMatchReg(address,
                                                                               (uint32) USER_PORT_addrBuf2);
    }
    else
    {
        uint32 matchReg;
        ...
        matchReg |= ((uint32)  USER_PORT_GET_I2C_8BIT_ADDRESS(address)); /* Set mask */
...
/*******************************************************************************
* Function Name: USER_PORT_EzI2CGetAddress1
********************************************************************************
*
* Summary:
...
uint32 USER_PORT_EzI2CGetAddress1(void)
{
    ...
    if(USER_PORT_SECONDARY_ADDRESS_ENABLE)
    {
        address = (uint32) USER_PORT_addrBuf1;
    }
    else
    {
        address = (USER_PORT_GET_RX_MATCH_ADDR(USER_PORT_RX_MATCH_REG) >>
...
/*******************************************************************************
* Function Name: USER_PORT_EzI2CSetBuffer1
********************************************************************************
*
* Summary:
...
void USER_PORT_EzI2CSetBuffer1(uint32 bufSize, uint32 rwBoundary, volatile uint8 * buffer)
{
    if (NULL != buffer)
    {
        ...
        USER_PORT_dataBuffer1 =  buffer;
        USER_PORT_bufSizeBuf1 = (uint16) bufSize;
        USER_PORT_protectBuf1 = (uint16) rwBoundary;
...
/*******************************************************************************
* Function Name: USER_PORT_EzI2CSetReadBoundaryBuffer1
********************************************************************************
*
* Summary:
...
void USER_PORT_EzI2CSetReadBoundaryBuffer1(uint32 rwBoundary)
{
...
#if(USER_PORT_SECONDARY_ADDRESS_ENABLE_CONST)
    /*******************************************************************************
    * Function Name: USER_PORT_EzI2CUpdateRxMatchReg
    ********************************************************************************
    *
    * Summary:
    ...
    *******************************************************************************/
    static uint32 USER_PORT_EzI2CUpdateRxMatchReg(uint32 addr1, uint32 addr2)
    {
        ...
        matchReg  = (uint32) (USER_PORT_GET_I2C_8BIT_ADDRESS(matchReg) << USER_PORT_RX_MATCH_MASK_POS);
        matchReg |= USER_PORT_GET_I2C_8BIT_ADDRESS(addr1);

        return(matchReg);
    }

    /*******************************************************************************
    * Function Name: USER_PORT_EzI2CSetAddress2
    ********************************************************************************
    *
    * Summary:
    ...
    *******************************************************************************/
    ...
    {
        USER_PORT_addrBuf2 = (uint8) address;

        USER_PORT_RX_MATCH_REG = USER_PORT_EzI2CUpdateRxMatchReg((uint32) USER_PORT_addrBuf1,
                                                                               address);
    }


    /*******************************************************************************
    * Function Name: USER_PORT_EzI2CGetAddress2
    ********************************************************************************
    *
    * Summary:
    ...
    *******************************************************************************/
    ...
    {
        return((uint32) USER_PORT_addrBuf2);
    }


    /*******************************************************************************
    * Function Name: USER_PORT_EzI2CSetBuffer2
    ********************************************************************************
    *
    * Summary:
    ...
    *******************************************************************************/
    void USER_PORT_EzI2CSetBuffer2(uint32 bufSize, uint32 rwBoundary, volatile uint8 * buffer)
    {
        if (NULL != buffer)
        {
            ...
            USER_PORT_dataBuffer2 =  buffer;
            USER_PORT_bufSizeBuf2 = (uint16) bufSize;
            USER_PORT_protectBuf2 = (uint16) rwBoundary;
    ...
    /*******************************************************************************
    * Function Name: USER_PORT_EzI2CSetReadBoundaryBuffer2
    ********************************************************************************
    *
    * Summary:
    *  Sets the read only boundary in the data buffer to be exposed to the I2C
    ...
    *******************************************************************************/
    ...
    /*******************************************************************************
    * Function Name: USER_PORT_EzI2CSaveConfig
    ********************************************************************************
    *
    * Summary:
    ...
    *******************************************************************************/
    void USER_PORT_EzI2CSaveConfig(void)
    {
        ...
        if(USER_PORT_EZI2C_SCL_STRETCH_ENABLE)
        {
        ...
        enableInt = (uint8) USER_PORT_INTR_I2C_EC_WAKE_UP;
        ...
        if(USER_PORT_EZI2C_SCL_STRETCH_ENABLE)
        {
            ...
            USER_PORT_SetI2CExtClkInterruptMode(enableInt);
        }
        else
        {
            for(;;) /* Wait for end of transaction intended to slave */
            {
                if(0u == (USER_PORT_GetI2CExtClkInterruptSource() & USER_PORT_INTR_I2C_EC_WAKE_UP))
                {
                    ...
                    if(0u == (USER_PORT_GetI2CExtClkInterruptSource() & USER_PORT_INTR_I2C_EC_WAKE_UP))
                    {
                        ...
                        else
                        {
                            ...
                            USER_PORT_I2C_SLAVE_CLEAR_NACK;
                        }
                    }
            ...
            USER_PORT_SetTxInterruptMode(USER_PORT_NO_INTR_SOURCES);
            USER_PORT_SetSlaveInterruptMode(USER_PORT_NO_INTR_SOURCES);
            USER_PORT_ClearPendingInt();
            USER_PORT_EnableInt();
    ...
    /*******************************************************************************
    * Function Name: USER_PORT_EzI2CRestoreConfig
    ********************************************************************************
    *
    * Summary:
    ...
    void USER_PORT_EzI2CRestoreConfig(void)
    {
        ...
        else
        {
            ...
            */
            ...
            USER_PORT_SetI2CExtClkInterruptMode(USER_PORT_NO_INTR_SOURCES);
            USER_PORT_SetSlaveInterruptMode(USER_PORT_EZI2C_INTR_SLAVE_MASK |
