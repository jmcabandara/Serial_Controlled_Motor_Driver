...
{
    CyRamVectors[CYINT_IRQ_BASE + InputInterrupt__INTC_NUMBER] = address;
}
...
{
    return CyRamVectors[CYINT_IRQ_BASE + InputInterrupt__INTC_NUMBER];
}
...
void InputInterrupt_SetPriority(uint8 priority)
{
    ...
    uint32 priorityOffset = ((InputInterrupt__INTC_NUMBER % 4u) * 8u) + 6u;
    
	interruptState = CyEnterCriticalSection();
    *InputInterrupt_INTC_PRIOR = (*InputInterrupt_INTC_PRIOR & (uint32)(~InputInterrupt__INTC_PRIOR_MASK)) |
...
uint8 InputInterrupt_GetPriority(void)
{
    ...
    priority = (*InputInterrupt_INTC_PRIOR & InputInterrupt__INTC_PRIOR_MASK) >> priorityOffset;
