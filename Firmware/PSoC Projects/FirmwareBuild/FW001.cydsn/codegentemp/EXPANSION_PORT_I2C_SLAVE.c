/*******************************************************************************
* File Name: EXPANSION_PORT_I2C_SLAVE.c
* Version 2.0
*
* Description:
...
volatile uint8 EXPANSION_PORT_slStatus;          /* Slave Status */

/* Receive buffer variables */
volatile uint8 * EXPANSION_PORT_slWrBufPtr;      /* Pointer to Receive buffer  */
volatile uint32  EXPANSION_PORT_slWrBufSize;     /* Slave Receive buffer size  */
volatile uint32  EXPANSION_PORT_slWrBufIndex;    /* Slave Receive buffer Index */

/* Transmit buffer variables */
volatile uint8 * EXPANSION_PORT_slRdBufPtr;      /* Pointer to Transmit buffer  */
volatile uint32  EXPANSION_PORT_slRdBufSize;     /* Slave Transmit buffer size  */
volatile uint32  EXPANSION_PORT_slRdBufIndex;    /* Slave Transmit buffer Index */
volatile uint32  EXPANSION_PORT_slRdBufIndexTmp; /* Slave Transmit buffer Index Tmp */
volatile uint8   EXPANSION_PORT_slOverFlowCount; /* Slave Transmit Overflow counter */


/*******************************************************************************
* Function Name: EXPANSION_PORT_I2CSlaveStatus
********************************************************************************
*
* Summary:
...
*
* Global variables:
*  EXPANSION_PORT_slStatus - used to store current status of I2C slave.
*
*******************************************************************************/
uint32 EXPANSION_PORT_I2CSlaveStatus(void)
{
    return((uint32) EXPANSION_PORT_slStatus);
}


/*******************************************************************************
* Function Name: EXPANSION_PORT_I2CSlaveClearReadStatus
********************************************************************************
*
* Summary:
...
uint32 EXPANSION_PORT_I2CSlaveClearReadStatus(void)
{
    ...
    status = ((uint32) EXPANSION_PORT_slStatus & EXPANSION_PORT_I2C_SSTAT_RD_MASK);
    EXPANSION_PORT_slStatus &= (uint8) ~EXPANSION_PORT_I2C_SSTAT_RD_CLEAR;
...
/*******************************************************************************
* Function Name: EXPANSION_PORT_I2CSlaveClearWriteStatus
********************************************************************************
*
* Summary:
...
uint32 EXPANSION_PORT_I2CSlaveClearWriteStatus(void)
{
    ...
    /* Mask of transfer complete flag and Error status */
    status = ((uint32) EXPANSION_PORT_slStatus & EXPANSION_PORT_I2C_SSTAT_WR_MASK);
    EXPANSION_PORT_slStatus &= (uint8) ~EXPANSION_PORT_I2C_SSTAT_WR_CLEAR;
...
/*******************************************************************************
* Function Name: EXPANSION_PORT_I2CSlaveSetAddress
********************************************************************************
*
* Summary:
...
void EXPANSION_PORT_I2CSlaveSetAddress(uint32 address)
{
    ...
    matchReg |= ((uint32)  EXPANSION_PORT_GET_I2C_8BIT_ADDRESS(address));
...
/*******************************************************************************
* Function Name: EXPANSION_PORT_I2CSlaveSetAddressMask
********************************************************************************
*
* Summary:
...
/*******************************************************************************
* Function Name: EXPANSION_PORT_I2CSlaveInitReadBuf
********************************************************************************
*
* Summary:
...
*
* Global variables:
*  EXPANSION_PORT_slRdBufPtr   - used to store pointer to slave read buffer.
*  EXPANSION_PORT_slRdBufSize  - used to store salve read buffer size.
*  EXPANSION_PORT_slRdBufIndex - used to store current index within slave
*  read buffer.
*
* Side Effects:
...
void EXPANSION_PORT_I2CSlaveInitReadBuf(uint8 * rdBuf, uint32 bufSize)
{
    ...
    if(NULL != rdBuf)
    {
        ...
        EXPANSION_PORT_slRdBufPtr      = (volatile uint8 *) rdBuf; /* Set buffer pointer  */
        EXPANSION_PORT_slRdBufSize     = bufSize;                  /* Set buffer size     */
        EXPANSION_PORT_slRdBufIndex    = 0u;                       /* Clear buffer index  */
        EXPANSION_PORT_slRdBufIndexTmp = 0u;                       /* Clear buffer index  */
...
/*******************************************************************************
* Function Name: EXPANSION_PORT_I2CSlaveInitWriteBuf
********************************************************************************
*
* Summary:
*  Sets the buffer pointer and size of the read buffer. This function also
...
*
* Global variables:
*  EXPANSION_PORT_slWrBufPtr   - used to store pointer to slave write buffer.
*  EXPANSION_PORT_slWrBufSize  - used to store salve write buffer size.
*  EXPANSION_PORT_slWrBufIndex - used to store current index within slave
*  write buffer.
*
* Side Effects:
*  If this function is called during a bus transaction, data from the previous
...
void EXPANSION_PORT_I2CSlaveInitWriteBuf(uint8 * wrBuf, uint32 bufSize)
{
    ...
    if(NULL != wrBuf)
    {
        ...
        EXPANSION_PORT_slWrBufPtr   = (volatile uint8 *) wrBuf; /* Set buffer pointer  */
        EXPANSION_PORT_slWrBufSize  = bufSize;                  /* Set buffer size     */
        EXPANSION_PORT_slWrBufIndex = 0u;                       /* Clear buffer index  */
...
/*******************************************************************************
* Function Name: EXPANSION_PORT_I2CSlaveGetReadBufSize
********************************************************************************
*
* Summary:
...
{
    return(EXPANSION_PORT_slRdBufIndex);
}


/*******************************************************************************
* Function Name: EXPANSION_PORT_I2CSlaveGetWriteBufSize
********************************************************************************
*
* Summary:
...
{
    return(EXPANSION_PORT_slWrBufIndex);
}


/*******************************************************************************
* Function Name: EXPANSION_PORT_I2CSlaveClearReadBuf
********************************************************************************
*
* Summary:
...
/*******************************************************************************
* Function Name: EXPANSION_PORT_I2CSlaveClearWriteBuf
********************************************************************************
*
* Summary:
